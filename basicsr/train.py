...
def init_loggers(opt):
    ...
    if opt['logger'].get('use_tb_logger'):
        save_path = opt['path'].get('save_path', '')
        tb_logger = init_tb_logger(log_dir=osp.join(save_path, 'tb_logger', opt['name']))
...
def train_pipeline(root_path):
    ...
    if resume_state is None:
        ...
        if opt['logger'].get('use_tb_logger') and opt['rank'] == 0:
            save_path = opt['path'].get('save_path', '')
            mkdir_and_rename(osp.join(save_path, 'tb_logger', opt['name']))
